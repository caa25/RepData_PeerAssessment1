...
We also know that there are `r 24 * 60` minutes per day, assuming a 24 hour period (24 hours/day * 60 min/hour = 1,440 minutes/day).  We also know that the data was collected at 5 minute intervals, meaning that there should be `r 1440 / 5` observations for each day (1,440 min/day / 5 minute = 288 records).  Finally, we know that there are 17,568 observations in the dataset.  17,568 observations / 288 observations per day = `r 17568 / 288` days, which is consistent with our findings above. We will see additional proof that there are 288 observations in our analysis, below.
...
There are no missing date or interval values, but there are 2,304 missing steps values coded as NA, representing `r colMeans(is.na(source_dataset))[1] * 100`% of the observations.
...
```
The mean number of steps per day is `r formatC(mean(plotdata1$Steps, na.rm = TRUE), format = "f")`.
The median number of steps per day is `r median(plotdata1$Steps, na.rm = TRUE)`. 
...
The new mean number of steps per day is `r formatC(mean(plotdata3$Steps, na.rm = TRUE), format = "f")`.
The new median number of steps per day is `r formatC(median(plotdata3$Steps, na.rm = TRUE), format = "f")`.
